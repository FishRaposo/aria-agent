...
legacy ``aria.internal.vendor_core.llm.MODEL_COSTS`` per-1M) into one per-1M registry, and supports
an external JSON/YAML override file so deployments can adjust prices without code
changes. ``aria.internal.vendor_core.llm.estimate_llm_cost`` delegates here.
"""
...
# claude-3-5-sonnet and claude-3-haiku match the legacy aria.internal.vendor_core.llm.MODEL_COSTS
