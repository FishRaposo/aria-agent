...
def estimate_llm_cost(model: str, prompt_tokens: int, completion_tokens: int) -> float:
    ...
    Delegates to :mod:`aria.internal.vendor_core.pricing`, the single source of truth for
    per-model token pricing. ``MODEL_COSTS`` above is retained for backward
    compatibility; new code should prefer ``aria.internal.vendor_core.pricing``.
    """
